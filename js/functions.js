...
const api_key = '9f37630e1760ca465317ad489651d9ce';
            ...
            error => {
                switch(error.code) {
                    case error.PERMISSION_DENIED:
                        alert("User denied the request for Geolocation. Please allow access to your location.");
                        break;
...
const getWeather = (lat, lng) => {
    ...
    axios.get(address)
        .then(response => {
            ...
            const image = 'https://openweathermap.org/img/wn/' + json.weather[0].icon + '@2x.png';
            icon_img.src = image;
        }).catch(error => {
            alert(error);
        });
